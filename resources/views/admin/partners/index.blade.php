...
@section('title', 'Parceiros | AngoMarketers CMS')
@section('page_title', 'Gestão de Parceiros')
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4">
        <form action="{{ route('admin.partners.index') }}" method="GET" class="flex items-center gap-3">
            <input type="text" name="search" value="{{ $search }}" placeholder="Pesquisar por nome de parceiro..."
            ...
            @if($search)
                <a href="{{ route('admin.partners.index') }}" class="text-xs text-slate-400 hover:text-slate-200 underline">Limpar</a>
            @endif
        </form>
    </div>
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-850/20">
                            ...
                            <td class="p-4">
                                <img src="{{ $partner->logo_path }}" 
                                     alt="{{ $partner->name }}" class="h-8 max-w-[80px] object-contain bg-slate-100 dark:bg-slate-800 p-1 border border-slate-200 dark:border-slate-700">
                            </td>
                            <!-- Name -->
                            ...
                            <td class="p-4 text-slate-500 dark:text-slate-450 font-mono text-xs">
                                <a href="{{ $partner->url }}" target="_blank" class="hover:text-primary hover:underline">
                                    {{ $partner->url ?? 'Nenhum' }}
                                </a>
                            </td>
                            <!-- Order -->
                            ...
                            <td class="p-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.partners.destroy', $partner) }}" method="POST" onsubmit="return confirm('Deseja realmente eliminar este parceiro?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
