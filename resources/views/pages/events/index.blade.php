...
@section('title', 'Eventos de Marketing, Tecnologia e Inovação em Angola | AngoMarketers')
                    ...
                    <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/60 dark:border-slate-800 shadow-xs hover:shadow-lg transition-all duration-300 flex flex-col h-full overflow-hidden group">
                        ...
                        <div class="p-6 flex-grow flex flex-col justify-between">
                            ...
                            <div class="mt-6 pt-5 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                                <a href="{{ route('events.show', $event->slug) }}" class="text-xs font-extrabold uppercase text-slate-900 dark:text-white hover:text-primary transition-colors flex items-center gap-1">
                                    Saber Mais &rarr;
                                </a>
                                @if($event->registration_link)
                                    <a href="{{ $event->registration_link }}" target="_blank" class="px-4 py-2 bg-primary hover:bg-primary/95 text-white text-xs font-bold rounded-xl shadow-md transition-all">
                                        Inscrever-se
                                    </a>
                                @else
                ...
                @foreach($pastEvents as $event)
                    <div class="bg-slate-50 dark:bg-slate-900/60 rounded-3xl border border-slate-100 dark:border-slate-800/80 p-6 flex flex-col sm:flex-row gap-6 shadow-xs hover:shadow-md transition-shadow group">
                        ...
                        <div class="relative w-full sm:w-48 h-36 rounded-2xl overflow-hidden shrink-0 bg-slate-200">
                            <img src="{{ $event->image_path ?: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=300' }}" 
                        ...
                        <div class="flex-grow flex flex-col justify-between">
                            <div>
                                <span class="text-[9px] font-extrabold uppercase tracking-widest text-slate-400 dark:text-slate-500">Resumo do Evento • {{ $event->event_date->format('d/m/Y') }}</span>
                            ...
                            <div class="mt-4 flex items-center gap-4">
                                <a href="{{ route('events.show', $event->slug) }}" class="text-xs font-bold text-primary hover:underline flex items-center gap-1">
