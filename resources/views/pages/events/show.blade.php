...
@section('title', $event->title . ' | Eventos AngoMarketers')
...
<div class="max-w-7xl mx-auto px-4 md:px-8 py-8">
        ...
        <ol class="inline-flex items-center space-x-1 md:space-x-2">
            ...
            <li>
                <div class="flex items-center">
                    ...
                    <a href="{{ route('events.index') }}" class="hover:text-primary">Eventos</a>
                </div>
            </li>
            <li aria-current="page">
                <div class="flex items-center">
                    <span class="mx-1.5 md:mx-2.5">/</span>
                    <span class="text-slate-600 dark:text-slate-300 truncate max-w-[200px] md:max-w-xs">{{ $event->title }}</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center mb-12">
        ...
        <div class="lg:col-span-5 space-y-6">
            <span class="px-3.5 py-1.5 bg-primary/10 text-primary text-[10px] font-black uppercase tracking-widest rounded-full inline-block">
                {{ $event->isPast() ? 'Evento Realizado' : 'Próximo Evento' }}
            </span>
            ...
            <div class="space-y-3 pt-2">
                <div class="flex items-center gap-3 text-xs font-semibold text-slate-600 dark:text-slate-300">
                    ...
                    <div>
                        ...
                        {{ $event->event_date->format('d \d\e F \d\e Y \à\s H:i') }}
                    </div>
                </div>
            ...
            <div class="pt-4 flex flex-wrap gap-4">
                @if(!$event->isPast())
                    @if($event->registration_link)
                        <a href="{{ $event->registration_link }}" target="_blank" class="px-6 py-3 bg-primary hover:bg-primary/95 text-white text-xs font-extrabold uppercase tracking-wider rounded-xl shadow-lg transition-all">
        ...
        <div class="lg:col-span-7 bg-slate-900 rounded-3xl overflow-hidden shadow-2xl aspect-video border border-slate-200/40 dark:border-slate-800">
            <img src="{{ $event->image_path ?: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=1200' }}" 
                 alt="{{ $event->title }}" 
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-8 space-y-12">
            ...
            @if($event->isPast() && $event->video_url)
                @php
                    ...
                    <div class="relative z-10">
                        ...
                        @else
                            <div class="aspect-video bg-slate-900 rounded-2xl flex items-center justify-center border border-slate-800">
                                <a href="{{ $event->video_url }}" target="_blank" class="text-sm font-bold text-primary hover:underline flex items-center gap-2">
            ...
            @if($event->isPast() && count($event->photos) > 0)
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4" x-data="{ openImage: null }">
                        @foreach($event->photos as $photo)
                            <div class="aspect-square bg-slate-100 dark:bg-slate-800 rounded-2xl overflow-hidden cursor-pointer shadow-xs border border-slate-200/40 dark:border-slate-800 group"
                                 x-on:click="openImage = '{{ $photo->image_path }}'">
                                <img src="{{ $photo->image_path }}" class="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-300" />
                            </div>
                        @endforeach
                            ...
                            @foreach($upcomingEvents as $upEvent)
                                <div class="flex gap-3 items-start group">
                                    ...
                                    <div class="w-11 h-12 bg-primary/10 rounded-lg flex flex-col items-center justify-center text-primary shrink-0">
                                        <span class="text-xs font-black leading-none">{{ $upEvent->event_date->format('d') }}</span>
                                        <span class="text-[8px] font-extrabold uppercase leading-none mt-1">{{ $upEvent->event_date->format('M') }}</span>
                                    </div>
                                    <div class="min-w-0">
                                        ...
                                        <span class="text-[9px] text-slate-400 font-semibold block mt-1">{{ $upEvent->location }}</span>
                                    </div>
                                </div>
                            @endforeach
                        </div>
