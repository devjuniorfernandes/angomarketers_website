...
<div {{ $attributes->merge(['class' => 'p-6 sm:p-8 bg-slate-50 dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-6 shadow-xs']) }}>
    <!-- Avatar image -->
    <a href="/author/{{ $slug }}" class="w-18 h-18 sm:w-20 sm:h-20 rounded-full overflow-hidden shrink-0 border-2 border-slate-200 dark:border-slate-800">
    ...
    <div class="flex-grow text-center sm:text-left">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 mb-2">
            <div>
                <span class="text-[10px] font-extrabold uppercase tracking-widest text-primary leading-none">{{ $role }}</span>
                <h4 class="text-lg font-extrabold text-slate-900 dark:text-white mt-1 leading-tight">
                    <a href="/author/{{ $slug }}" class="hover:text-primary transition-colors">
                        {{ $name }}
                    </a>
                </h4>
            </div>
