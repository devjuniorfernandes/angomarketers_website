    ...
    <div :class="scrolled ? 'h-0 -translate-y-full opacity-0' : 'h-10 opacity-100'" 
        ...
        <div class="flex-grow overflow-hidden relative h-5">
            <div class="absolute inset-0 flex items-center whitespace-nowrap gap-12 animate-[marquee_25s_linear_infinite] hover:[animation-play-state:paused]">
                @foreach(array_merge($tickerItems, $tickerItems) as $item)
                    <a href="{{ $item['url'] }}" class="text-slate-300 font-medium hover:text-primary transition-colors cursor-pointer">
                        <span class="text-slate-200 font-bold">{{ $item['category_name'] }}:</span> {{ $item['title'] }}
                    </a>
                @endforeach
            </div>
        </div>
        ...
        <div class="hidden md:flex items-center gap-4 shrink-0 pl-4 border-l border-slate-800">
            <div class="flex items-center gap-3">
                <a href="{{ $facebookUrl }}" target="_blank" class="text-slate-400 hover:text-white transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
                <a href="{{ $instagramUrl }}" target="_blank" class="text-slate-400 hover:text-white transition-colors">
                ...
                <a href="{{ $linkedinUrl }}" target="_blank" class="text-slate-400 hover:text-white transition-colors">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                </a>
            </div>
            <a href="{{ route('home') }}#newsletter-form" class="bg-slate-800 hover:bg-slate-700 text-[10px] font-bold tracking-wider uppercase py-1 px-3.5 rounded border border-slate-700 transition-colors">
                Newsletter
            </a>
        </div>
    </div>
    ...
    <nav :class="scrolled ? 'bg-white/95 dark:bg-slate-950/95 shadow-md border-b border-slate-200/80 dark:border-slate-800/85 backdrop-blur-md py-3.5' : 'bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 py-5'" 
        ...
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <ul class="hidden lg:flex items-center gap-8 xl:gap-10 font-heading text-sm font-bold text-slate-700 dark:text-slate-300">
                @foreach($navItems as $item)
                    @if($item['key'])
                        <li class="relative" 
                            x-on:mouseenter="activeMenu = '{{ $item['key'] }}'">
                            <a href="{{ $item['url'] }}" 
                               class="flex items-center gap-1.5 py-2 hover:text-primary transition-colors uppercase tracking-wider"
                               :class="activeMenu === '{{ $item['key'] }}' ? 'text-primary' : ''">
                                {{ $item['title'] }}
                                <svg class="w-3.5 h-3.5 transition-transform duration-200" :class="activeMenu === '{{ $item['key'] }}' ? 'rotate-180 text-primary' : ''" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @if(count($item['children']) > 0)
                                ...
                                <div x-show="open"
                                    ...
                                    @foreach($item['children'] as $child)
                                        <a href="{{ $child->url }}" class="block px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-primary transition-colors">
                                            {{ $child->title }}
                                        </a>
                                    @endforeach
                                </div>
            ...
            <div class="flex items-center gap-2.5 sm:gap-4 text-slate-600 dark:text-slate-400">
                ...
                <a href="{{ route('home') }}#newsletter-form" class="hidden sm:inline-flex items-center justify-center px-5 py-2.5 bg-primary hover:bg-primary/90 text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-300 shadow-sm hover:shadow-lg">
                    Subscrever
                </a>
            ...
            <div class="max-w-7xl mx-auto grid grid-cols-12 gap-8">
                ...
                <div class="col-span-3">
                    ...
                    <ul class="space-y-3 font-heading text-sm font-semibold">
                        @foreach(\App\Domain\Categories\Models\Category::all() as $cat)
                            <li>
                                <a href="{{ route('category', $cat->slug) }}" class="text-slate-800 dark:text-slate-200 hover:text-primary transition-colors flex items-center gap-1.5">
                                    <span class="w-1.5 h-1.5 bg-primary rounded-full"></span> {{ $cat->name }}
                                </a>
                            </li>
                        @endforeach
                    </ul>
                ...
                <div class="col-span-9 grid grid-cols-3 gap-6 border-l border-slate-100 dark:border-slate-800 pl-8">
                    @foreach($headerArticles as $art)
                        <a href="{{ route('article', $art->slug) }}" class="group flex flex-col justify-between h-full">
                            <div>
                                <div class="aspect-video bg-slate-100 dark:bg-slate-800 overflow-hidden mb-3 relative">
                                    <img src="{{ $art->image_path ?: 'https://images.unsplash.com/photo-1498050108023-c5249f4df085?q=80&w=400' }}" 
                                ...
                                <span class="text-[9px] font-black uppercase text-primary tracking-widest">{{ $art->category->name ?? 'Geral' }}</span>
                            ...
                            <span class="text-[10px] text-slate-450 dark:text-slate-500 mt-2 block">{{ $art->published_at ? $art->published_at->format('d/m/Y') : '' }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <div x-show="activeMenu === 'trainings'" 
            ...
            <div class="max-w-7xl mx-auto grid grid-cols-12 gap-8">
                ...
                <div class="col-span-9 grid grid-cols-3 gap-6 border-l border-slate-100 dark:border-slate-800 pl-8">
                    @foreach($headerTrainings as $tr)
                        <a href="{{ route('trainings.show', $tr->slug) }}" class="group flex flex-col justify-between h-full">
                            <div>
                                <div class="aspect-video bg-slate-100 dark:bg-slate-800 overflow-hidden mb-3 relative">
                                    <img src="{{ $tr->cover_image ?: 'https://images.unsplash.com/photo-1516321318423-f06f85e504b3?q=80&w=400' }}" 
                                         class="w-full h-full object-cover group-hover:scale-103 transition-transform duration-500" 
                                         onerror="this.style.display='none'; this.nextElementSibling.classList.remove('hidden');" />
                                    <div class="absolute inset-0 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 hidden flex items-center justify-center"></div>
                                    <span class="absolute top-2 left-2 bg-slate-900/80 text-white text-[8px] font-black uppercase tracking-wider px-2 py-0.5">{{ $tr->mode }}</span>
                                </div>
                                <span class="text-[9px] font-black uppercase text-slate-400 tracking-widest block">{{ $tr->institution }}</span>
                                <h5 class="font-bold text-slate-900 dark:text-white group-hover:text-primary transition-colors text-sm line-clamp-2 mt-1 leading-snug">
                                    {{ $tr->title }}
                                </h5>
                            </div>
                            <span class="text-[10px] text-primary font-bold mt-2 block">{{ $tr->price > 0 ? number_format($tr->price, 2, ',', '.') . ' AOA' : 'Gratuito' }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
        ...
        <div x-show="activeMenu === 'events'" 
            ...
            <div class="max-w-7xl mx-auto grid grid-cols-12 gap-8">
                ...
                <div class="col-span-9 grid grid-cols-3 gap-6 border-l border-slate-100 dark:border-slate-800 pl-8">
                    @foreach($headerEvents as $evt)
                        <a href="{{ route('events.show', $evt->slug) }}" class="group flex flex-col justify-between h-full">
                            <div>
                                <div class="aspect-video bg-slate-100 dark:bg-slate-800 overflow-hidden mb-3 relative">
                                    <img src="{{ $evt->image_path ?: 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?q=80&w=400' }}" 
                                         class="w-full h-full object-cover group-hover:scale-103 transition-transform duration-500" 
                                         onerror="this.style.display='none'; this.nextElementSibling.classList.remove('hidden');" />
                                    <div class="absolute inset-0 bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 hidden flex items-center justify-center"></div>
                                </div>
                                <span class="text-[9px] font-black uppercase text-primary tracking-widest block">{{ $evt->location }}</span>
                                <h5 class="font-bold text-slate-900 dark:text-white group-hover:text-primary transition-colors text-sm line-clamp-2 mt-1 leading-snug">
                                    {{ $evt->title }}
                                </h5>
                            </div>
                            <span class="text-[10px] text-slate-450 dark:text-slate-500 mt-2 block">{{ $evt->event_date ? $evt->event_date->format('d/m/Y H:i') : '' }}</span>
                        </a>
                    @endforeach
                </div>
            </div>
    ...
    <div x-show="mobileMenuOpen" 
        ...
        <div x-show="mobileMenuOpen" 
                ...
                @foreach($navItems as $item)
                    <li x-data="{ open: false }">
                        <div class="flex items-center justify-between">
                            <a href="{{ $item['url'] }}" x-on:click="mobileMenuOpen = false" class="block py-1 hover:text-primary transition-colors uppercase tracking-wider">
                                {{ $item['title'] }}
                            </a>
                            @if(count($item['children']) > 0)
                        ...
                        @if(count($item['children']) > 0)
                            <ul x-show="open" class="pl-4 mt-2 space-y-2 font-normal text-sm border-l border-slate-100 dark:border-slate-800">
                                @foreach($item['children'] as $child)
                                    <li>
                                        <a href="{{ $child->url }}" x-on:click="mobileMenuOpen = false" class="block py-1 text-slate-600 dark:text-slate-400 hover:text-primary">
                                            {{ $child->title }}
                                        </a>
                                    </li>
                                @endforeach
            ...
            <div class="mt-auto pt-8 border-t border-slate-100 dark:border-slate-800">
                <a href="{{ route('home') }}#newsletter-form" x-on:click="mobileMenuOpen = false" class="w-full inline-flex items-center justify-center px-4 py-3 bg-primary text-white text-xs font-bold uppercase tracking-wider rounded-xl transition-all duration-200 shadow-sm">
